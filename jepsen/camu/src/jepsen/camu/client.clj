   ...
   (try
     (let [rf  (get opts :replication-factor 1)
           ...
           body (cond-> {:name       topic
                         ...
                         :retention  (get opts :topic-retention "24h")
...
(defn- mc-find-lines
  ([prefix] (mc-find-lines prefix "*.json"))
  ([prefix pattern]
   (let [{:keys [exit out]} (mc "find" (s3-key prefix) "--name" pattern)]
     (if (zero? exit)
       (->> (str/split-lines out)
            (remove str/blank?))
       []))))
                                ...
                                (fn [partition]
                                  (let [key (str "readiness-probe-" partition)
                                    ...
                                    (boolean
                                     (some (fn [n]
                                             (try
                                               (let [result (produce! n topic partition key value)]
