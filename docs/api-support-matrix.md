...
| Iceberg export | `export_enabled` topic (projected under `maintenance.parquet_export.warehouse`) | Supported | Supported | `Partial` | `test/integration/iceberg_export_test.go`, `internal/server/iceberg_pipeline_export_test.go` | Diskless export mirrors the classic pipeline (partition leader exports committed refs below the diskless watermark); the Iceberg table is read back through the iceberg package in the integration suite. Remaining gap is broader multi-node diskless export/retention interplay. |
| Typed topics | `schema` on topic create (`json`/`avro`/`protobuf` encodings) | Supported | Supported | `Partial` | `test/integration/typed_topic_test.go`, `internal/server/schema_validation_test.go`, `internal/iceberg/avro_test.go`, `internal/iceberg/protobuf_test.go` | Validation, dead-letter routing, and encode/decode round trips are covered; broader multi-encoding negative-path coverage is still thin. |
| Schema registry | embedded `_meta/schemas/` versioning + backward compatibility | Supported | Supported | `Verified` | `internal/server/schema_registry_test.go`, `internal/server/schema_registry_fixes_test.go` | Idempotent registration, crash-orphan healing, GC, deletion cleanup, and compatibility checks are covered. |
| Schema evolution | `POST /v1/topics/{topic}/schema` | Supported | Supported | `Verified` | `internal/server/handlers_topic_schema_test.go`, `internal/server/schema_registry_test.go` | Endpoint updates the topic config and advances the Iceberg table schema; incompatible changes are rejected with 400. |
