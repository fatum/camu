...
| Produce | `POST /v1/topics/{topic}/partitions/{id}/messages` | Supported | Supported | `Partial` | existing server tests, diskless integration, idempotency-related coverage | Full classic/diskless idempotent parity is still not fully matrixed. |
