...
| Parquet export / SQL | `export_enabled` topic + `POST /v1/sql` | Supported | Supported | `Partial` | `test/integration/typed_topic_test.go`, `internal/server/parquet_pipeline_export_test.go` | Diskless export mirrors the classic pipeline (partition leader exports committed refs below the diskless watermark); SQL visibility is verified for diskless in `typed_topic_test.go`. Remaining gap is broader multi-node diskless export/retention interplay. |
